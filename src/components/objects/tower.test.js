...
describe('Tower catalog', () => {
    it('has exactly 15 tower types, each with 5 levels', () => {
        expect(TOWER_TYPES.length).toBe(15);
        TOWER_TYPES.forEach(type => {
            expect(TOWER_DEFS[type].levels.length).toBe(5);
        });
    });

    it('has the expected category counts, including the 4 new specialist supports', () => {
        const categories = TOWER_TYPES.map(t => TOWER_DEFS[t].category);
        expect(categories.filter(c => c === CATEGORY.ATTACK).length).toBe(6);
    ...
    it('the global-reach tower is always in range, regardless of distance', () => {
        const farseer = new Tower(0, 0, 15);
        const farAwayEnemy = { mid: { x: 100000, y: 100000 } };
        expect(farseer.inRange(farAwayEnemy)).toBe(true);
    });
});
