...
const BASE_GLOBAL_ACHIEVEMENTS = [
    ...
    { id: 'boss_bestiary', name: 'Boss Bestiary', desc: 'Defeat all 6 boss archetypes across your career.', check: (s, c) => c.uniqueBossArchetypes >= 6 },
...
const EXTRA_GLOBAL_ACHIEVEMENTS = [
    ...[[25,'Quarter Century','wave_25'],[50,'Fifty Waves Strong','wave_50'],[75,'Endurance Expert','wave_75'],[100,'Century Defense','wave_100'],[150,'Endless Legend','wave_150']]
    ...
    ...[[1,'First Boss Down','boss_1'],[5,'Boss Breaker','boss_5'],[25,'Boss Executioner','boss_25'],[100,'Boss Exterminator','boss_100'],[250,'Boss Apocalypse','boss_250']]
        .map(([threshold,name,id]) => ({ id,name,desc:`Defeat ${threshold} bosses across all runs.`,category:'global',check:s=>s.bossKills>=threshold })),
    ...[[1,'First Perfect Map','three_star_1'],[25,'Quarter Atlas','three_star_25'],[50,'Half Atlas','three_star_50'],[75,'Master Cartographer','three_star_75'],[100,'Perfect Atlas','three_star_100']]
    ...
    ...[[5,'Ranked Initiate','ranked_5'],[10,'Ranked Regular','ranked_10'],[30,'Ranked Veteran','ranked_30'],[50,'Ranked Elite','ranked_50'],[75,'Ranked Legend','ranked_75']]
    ...
    ...[[1,'Daily Debut','daily_1'],[10,'Daily Ten','daily_10'],[25,'Daily Dedicated','daily_25'],[50,'Daily Specialist','daily_50'],[100,'Daily Century','daily_100']]
...
function mapAchievementsFor(map, mapIndex) {
    ...
    return [
        { id:`${prefix}_scout`,name:`${mapName}: Scout`,category:'map',mapIndex,tier:1,desc:`Reach wave 5 on ${mapName}.`,check:(s,c)=>(c.records[mapIndex]?.bestWave||0)>=5 },
        { id:`${prefix}_veteran`,name:`${mapName}: Veteran`,category:'map',mapIndex,tier:2,desc:`Reach wave 10 on ${mapName} on Hard difficulty or higher.`,check:(s,c)=>Math.max(c.records[mapIndex]?.difficultyBest?.normal||0,c.records[mapIndex]?.difficultyBest?.hard||0,c.records[mapIndex]?.difficultyBest?.challenge||0)>=10 },
        { id:`${prefix}_flawless`,name:`${mapName}: Flawless`,category:'map',mapIndex,tier:3,desc:`Reach wave 15 on ${mapName} without losing a life.`,check:(s,c)=>(c.records[mapIndex]?.flawlessBest||0)>=15 },
        { id:`${prefix}_versatile`,name:`${mapName}: Versatile`,category:'map',mapIndex,tier:4,desc:`Reach wave 10 on ${mapName} in 3 different game modes.`,check:(s,c)=>Object.values(c.records[mapIndex]?.modeBest||{}).filter(wave=>wave>=10).length>=3 },
        { id:`${prefix}_ranked`,name:`${mapName}: Ranked Master`,category:'map',mapIndex,tier:5,desc:`Reach wave 20 on ${mapName} in Ranked play.`,check:(s,c)=>(c.records[mapIndex]?.rankedBest||0)>=20 },
    ];
}
...
function achievementContext() {
    ...
    return {
        ...
        uniqueBossArchetypes: (progression.lifetime?.bossArchetypes || []).length,
        totalStars: (progression.mapStars || []).reduce((sum,value)=>sum+(value||0),0),
    };
}

export function getStats() {
    try { const raw=localStorage.getItem(STATS_KEY); return raw?{...DEFAULT_STATS,...JSON.parse(raw)}:{...DEFAULT_STATS}; }
    catch { return {...DEFAULT_STATS}; }
}
function saveStats(stats){try{localStorage.setItem(STATS_KEY,JSON.stringify(stats));}catch{/* unavailable */}}
export function getUnlockedIds(){try{const raw=localStorage.getItem(UNLOCKED_KEY);return raw?JSON.parse(raw):[];}catch{return[];}}
...
export function recordStat(key,value,mode='max'){
    ...
    const stats=getStats();stats[key]=mode==='add'?(stats[key]||0)+value:Math.max(stats[key]||0,value);saveStats(stats);return checkForNewUnlocks(stats);
}
export function refreshAchievementUnlocks(){if(isAdminTestMode())return[];return checkForNewUnlocks(getStats());}
function checkForNewUnlocks(stats){
    const unlocked=new Set(getUnlockedIds());const newlyUnlocked=[];const context=achievementContext();
    ACHIEVEMENTS.forEach(a=>{if(!unlocked.has(a.id)&&a.check(stats,context)){unlocked.add(a.id);newlyUnlocked.push(a);}});
    if(newlyUnlocked.length)saveUnlockedIds([...unlocked]);return newlyUnlocked;
}
export function isUnlocked(id){return getUnlockedIds().includes(id);}
export function resetAchievements(){try{localStorage.removeItem(STATS_KEY);localStorage.removeItem(UNLOCKED_KEY);}catch{/* unavailable */}}
