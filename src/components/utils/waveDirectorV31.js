...
function traitFor(wave, difficulty, modeKey, rng) {
    ...
    const modeMult = modeKey === 'chaos' ? 2.2 : modeKey === 'titan' ? 1.35 : modeKey === 'bossrush' ? 1.2 : 1;
    if (rng() > Math.min(0.72, baseChance * modeMult)) return null;
...
export function buildWavePlan({ wave, map, mapIndex = 0, difficulty, modeKey = 'classic', mechanic, seed = 'standard' }) {
    ...
    if (modeKey === 'overdrive') threatBudget *= 1.08;
    if (modeKey === 'titan') threatBudget *= 0.92;
    if (modeKey === 'bossrush') threatBudget *= 0.95;
    const maxUnits = Math.min(70, 5 + Math.ceil(wave * 1.65));
    const entries = [];
    ...
    return {
        ...
        theme,
        entries,
        spawnInterval: modeKey === 'overdrive' ? 0.58 : modeKey === 'titan' ? 0.9 : 0.72,
...
export function applyPlannedTrait(enemy, spec) {
    ...
    switch (spec.trait) {
        case 'shielded':
            enemy.shieldHP = Math.max(enemy.shieldHP || 0, Math.round(enemy.maxHealth * 0.34));
            break;
        case 'splitter':
            enemy.splitter = true;
            break;
        case 'hasted':
            ...
            break;
        case 'fortified':
            enemy.armor = (enemy.armor || 0) + 7;
            break;
        case 'regenerating':
            enemy.regenPerSecond = Math.max(enemy.regenPerSecond || 0, enemy.maxHealth * 0.055);
            break;
        case 'miniboss':
            enemy.maxHealth = Math.round(enemy.maxHealth * 2.8);
            enemy.health = enemy.maxHealth;
            enemy.atk = Math.max(2, Math.round(enemy.atk * 1.7));
            enemy.value = Math.round(enemy.value * 2.2);
            enemy.score = Math.round(enemy.score * 2.2);
            enemy.armor = (enemy.armor || 0) + 4;
            enemy.shieldHP = Math.round(enemy.maxHealth * 0.16);
...
export function summarizeWavePlan(plan) {
    ...
    for (const spec of plan?.entries || []) {
        const label = enemyDisplayName(spec);
